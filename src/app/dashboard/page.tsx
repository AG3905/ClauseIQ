...
import {
  ...
  ArrowUpRight, Shield, Clock, FileCheck, ArrowRight, Plus
} from "lucide-react";
...
export default function DashboardPage() {
  ...
  const [counselName, setCounselName] = useState("Counsel");
  const [userAnalyses, setUserAnalyses] = useState<AnalysisItem[]>([]);
  ...
  useEffect(() => {
    ...
    async function loadDashboardData() {
      try {
        ...
        if (user) {
          const name = user.user_metadata?.full_name || getDisplayName("", user.email);
          setCounselName(name);
        } else {
          const localSession = getLocalAuthSession();
          if (localSession) {
            setCounselName(getDisplayName(localSession.full_name, localSession.email));
          }
        }

        const items = await fetchUserAnalyses();
        setUserAnalyses(items);
      } catch {
        // Fallback empty
      } finally {
        setLoading(false);
  ...
  const highExposureCount = userAnalyses.filter(a => a.verdict === "danger").length;
      ...
      <MotionSection amount={0.2} className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-border">
        <MotionItem>
          <div className="flex items-center gap-2 font-mono text-xs text-[#8C6721] dark:text-[#C99A52] uppercase tracking-wider mb-1">
            ...
            <span>Workspace Register • {counselName}</span>
          </div>
            ...
            <CardContent className="p-0 divide-y divide-border">
              {loading ? (
                ...
                <div className="p-8 text-center text-muted-foreground space-y-2">
                  <p className="font-serif text-sm font-semibold text-foreground">No contracts audited yet</p>
                  <p className="text-xs font-sans text-muted-foreground">
                    Upload your first legal agreement to populate your contract intelligence register.
                  </p>
                  <div className="pt-2">
                    <Link href="/dashboard/analyze">
                      <Button className="bg-[#8C6721] hover:bg-[#6E4E1C] dark:bg-[#C99A52] dark:hover:bg-[#B38743] text-white dark:text-[#171512] text-xs font-semibold h-8 px-3">
                        <Plus className="w-3.5 h-3.5 mr-1" /> Analyze Contract
                      </Button>
                    </Link>
                  </div>
                </div>
              ) : (
                userAnalyses.slice(0, 5).map((item) => (
                  <div key={item.id} className="flex flex-col sm:flex-row sm:items-center justify-between p-4 px-6 hover:bg-muted/40 transition-colors gap-3 group">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-sm font-serif font-semibold text-foreground group-hover:text-[#8C6721] dark:group-hover:text-[#C99A52] transition-colors">{item.document_name}</p>
                    ...
                    <div className="flex items-center gap-3 font-mono text-xs">
                      <span className="text-muted-foreground">Risk: {item.riskScore}</span>
                      <span className={`px-2.5 py-0.5 rounded text-[10px] font-medium uppercase ${riskBadgeStyle[item.verdict] || riskBadgeStyle.review}`}>
